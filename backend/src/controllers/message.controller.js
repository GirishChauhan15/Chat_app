...
const getAllMessages = asyncHandler(async(req,res)=> {
    ...
    let getAllMessageInfo = await Message.aggregatePaginate(messagesInfo, options)

    if(!getAllMessageInfo) {
        return res.status(500).json(new ApiError(500, "Failed to fetch messages, try later."))
    }
...
const sendMessage = asyncHandler(async(req,res)=> {
    ...
    if(message && message?.length) {
        if(message?.length > 300) {
            removeImageFromServer(file?.path)
            return res.status(400).json(new ApiError(400, "Message too long. Max 300 characters."))
        }
    }
    
    if(!file?.path && !message?.trim()) {
        return res.status(400).json(new ApiError(400, "Message or image field is required."))
    }
    ...
    if(file?.path) {
        ...
        if(!cloudinaryResponse) {
             removeImageFromServer(file?.path)
            return res.status(500).json(new ApiError(500, "Failed to upload image, try later."))
        }
        
        // Encrypts Image
        let encryptedImageUrl = encryptData(cloudinaryResponse?.secureUrl, process.env.IMAGE_ENCRYPTION_KEY)
        
        if(!encryptedImageUrl?.success) {
            removeImageFromServer(file?.path)
            return res.status(500).json(new ApiError(500, "Failed to encrypt image, try later."))
        }
    ...
    if(message) {
        encryptedMessage = encryptData(message, process.env.MESSAGE_ENCRYPTION_KEY)
    }
    
    if(encryptedMessage && !encryptedMessage?.success) {
        return res.status(500).json(new ApiError(500, "Failed to encrypt message, try later."))
    }
    ...
    if(!newMessage) return res.status(500).json(new ApiError(500, "Failed to send message, try later."))

    const messageInfo = await Message.aggregate([
        { 
            $match : {
    ...
    if(userStatus?.userId === receiverId) {
        io.to(userStatus?.socketId).emit('newMessage', messageInfo[0])
    }
    
    if(!messageInfo) {
        return res.status(500).json(new ApiError(500, "Failed to send message, try later."))
    }
    return res.status(201).json(new ApiResponse(201, messageInfo[0], "Message send successfully."))
})
